function LiDAR_Data_Output = fcn_Transform_transformVehicleToLiDAR(LiDAR_Data,RearRightGPS_offset_relative_to_VehicleOrigin,...
...
M_transform_LiDAR_to_Vehicle = fcn_Transform_calculateTransformation_LiDARVelodyneToVehicle( ...
            ...
            M_transform_LiDARVelodyne_to_RearRightGPS);
M_transform_Vehicle_to_LiDAR = inv(M_transform_LiDAR_to_Vehicle);
...
for ith_frame = 1:N_frames
    ...
    if ~isempty(PointCloud_ith_frame)
        XYZ_ith_frame_Vehicle= PointCloud_ith_frame(:,1:3);
        XYZ_ith_frame_LiDAR= se3(M_transform_Vehicle_to_LiDAR).transform(XYZ_ith_frame_Vehicle);
        transformed_PointCloud_ith_frame = [XYZ_ith_frame_LiDAR, PointCloud_ith_frame(:,4:end)];
    end
