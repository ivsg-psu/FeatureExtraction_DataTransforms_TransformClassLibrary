function transformed_SensorCoord_in_ENU = fcn_Transform_SensorCoordToENU(vehicleParameters, sensorPoseParameters, sensor_or_vehicle, vehiclePose_ENU, sensorReading_SensorCoord, varargin)
...
% Check the sizes of sensorReading_SensorCoord and vehiclePose_ENU, and
% duplicate the vehiclePose_ENU based on the size(,1) of
% sensorReading_SensorCoord.
if max(size(sensorReading_SensorCoord)) > max(size(vehiclePose_ENU))
    vehiclePose_ENU = fcn_Transform_generateVehiclePoseDuplicates(vehiclePose_ENU, sensorReading_SensorCoord);
end
...
transformed_SensorCoord_in_ENU = zeros(size(vehiclePose_ENU,1),3);
...
for i = 1:size(vehiclePose_ENU,1)
    ...
    transformed_SensorCoord_in_ENU(i,:) = transformed_SensorCoordPoint_in_ENU_homogeneous(1:3,:)';  
end
